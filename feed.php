<?php 
    ...
    array_walk($faculty, 'trim_value');
    array_walk($subs, 'trim_value');
    $year = $_REQUEST['year'];
    ...
    $query = "select * from rating where roll_number='$roll_no'";
    $hasVoted = $conn->query($query);
    if($hasVoted->num_rows > 0) {
        $sql = "DELETE FROM rating where roll_number='$roll_no'";
